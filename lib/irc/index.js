	...
	webhook: function(client, headers, body) {
		...
		if (headers['x-github-event']) {
			...
			switch (headers['x-github-event']) {
				case "push":
					message += body.commits[0].author.username + " just pushed " + body.commits[0].url;
					break;
				case "pull_request":
					message += body.pull_request.user.login + " just ";
					if (body.action === 'closed' && body.pull_request.merged_at !== null) message += "merged";
					else message += body.action + " ";
					message += body.pull_request.url;
					break;
				case "commit_comment":
					message += body.comment.user.login + " just commented on " + body.comment.url;
					break;
				case "issue_comment":
					message += body.issue.user.login + " just commented on " + body.issue.url;
					break;
				case "pull_request_review_comment":
					message += body.comment.user.login + " just commented on " + body.comment.url;
					break;
				case "issues":
					message += body.issue.user.login + " just " + body.action + " " + body.pull_request.url;
					break;
			}
			console.log(message);
			client.get_channel(this.__profile[0].channels[0]).send(message);
