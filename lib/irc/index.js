...
Bot.prototype.compile_command_listener = function() {
	...
	if (this.__trigger_changed) {
		...
		this.__commands_regex = new RegExp("^\.([A-Za-z]+)\s?(.*)?)$");
...
Bot.prototype.listeners = {
	...
	message: function(channel, user, text) {
		...
		if (text.match(/^tell/)) {
			var regex_split = text.match(/^tell\s([A-Za-z0-9_-]+)\sabout\s(.*)$/i);
			var intent = regex_split[1];
			var command = regex_split[2];
			...
			if (this.__commands.hasOwnProperty(command)) {
				this.__commands[command].callback.call(this, context, 'none', command);
			}
			else {
				this.emit('command_not_found', context, full);
			}
			...
			var regex_split = text.match(/^msg\s([A-Za-z0-9_-]+)\sabout\s(.*)$/i);
			var intent = regex_split[1];
			var command = regex_split[2];
		...
		else if (text.match(/^ciu/)) {
			var regex_split = text.match(/^ciu\s(.*)$/i);
			var query = regex_split[1];
			var command = 'caniuse';
			...
			if (this.__commands.hasOwnProperty(command)) {
				context.intent = context.sender.name;
				this.__commands[command].callback.call(this, context, query, command);
			}
			else {
				this.emit('command_not_found', context, full);
			}
		...
		else if (text.match(/^mdn/)) {
			var regex_split = text.match(/^mdn\s(.*)$/i);
			var query = regex_split[1];
			var command = 'mdn';
		...
		else if (text.match(/^wpd/)) {
			var regex_split = text.match(/^wpd\s(.*)$/i);
			var query = regex_split[1];
			var command = 'wpd';
		...
		else if (text.match(/^validate/)) {
			var regex_split = text.match(/^validate\s(.*)$/i);
			var url = regex_split[1];
			...
			if (this.__commands.hasOwnProperty(command)) {
				context.priv = true;
				this.__commands[command].callback.call(this, context, url, command);
			}
			else {
				this.emit('command_not_found', context, full);
			}
		...
		else if (text.match(/^shorten/)) {
			var regex_split = text.match(/^shorten\s(.*)$/i);
			var url = regex_split[1];
			var command = 'shorten';
			...
			var command_matches = text.match(/^\.([A-Za-z])\s?(.*)?$/);
			if (command_matches) {
				var command = command_matches[1].toLowerCase();
				var parameters = command_matches[2];
				...
				if (this.__commands.hasOwnProperty(command)) {
					...
					var intent_obj = Bot.prototype.parse_intent.call(this, command);
					if (intent_obj) {
						full = intent_obj.result;
						context.intent = channel.client.get_user(intent_obj.intent);
					}
					this.emit('command_not_found', context, command);
				}
				return;
			}
