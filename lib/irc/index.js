		...
		if (headers['x-github-event']) {
			...
			switch (headers['x-github-event']) {
				...
				case "pull_request":
					...
					break;
				case "commit_comment":
					message += body.comment.user.login + " just commented on " + body.comment.html_url;
					break;
				case "issue_comment":
					message += body.issue.user.login + " just commented on " + body.issue.html_url;
					break;
				case "pull_request_review_comment":
					message += body.comment.user.login + " just commented on " + body.comment.html_url;
					break;
				case "issues":
					message += body.issue.user.login + " just " + body.action + " " + body.pull_request.html_url;
					break;
			}
			client.get_channel(this.__profile[0].channels[0]).send(message);
			return;
