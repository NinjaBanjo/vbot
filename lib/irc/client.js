...
var Client = module.exports = function(profile) {
	...
	this.travis_auth_key = profile.travis_key;
	this.repo_owner = profile.repo_owner;
	this.repo_name = profile.repo_name;
...
Client.prototype.authenticate = function() {
	if (typeof this.nickserv === "string") {
		...
		this.get_user("NickServ").send("identify " + this.nick + " " + this.password);
	}
};
...
Client.prototype.events = {
	secureConnect: function() {
		...
		if (this.profile.ssl_verify === false)
			authorized = true;
...
Client.Channel.prototype.notice = function(message, user) {
	this.client.raw("NOTICE " + user + " " + message);
};
...
Client.Channel.prototype.join_on_invite = function(channel) {
...
Client.Channel.prototype.kick = function(user) {
	this.client.raw("KICK " + this.name + " "+user);
};

Client.Channel.prototype.kickban = function(user, mask) {
	this.client.raw("MODE " + this.name + " +b" + " " + mask);
	this.client.raw("KICK " + this.name + " "+user);
};

Client.Channel.prototype.whois = function(user) {
	this.client.raw("WHOIS " + user);
};
...
Client.Webhook = function(client, port) {
	var app = express();
    ...
    app.post('/webhook', function(req, res) {
    	...
    	try {
    		...
    		console.log(e, "parsing");
    	}
    	finally {
    		body = req.body;
    	}
        client.emit('webhook', client, req.headers, body);
        res.status(200).end();
    });
...
util.inherits(Client.Webhook, events.EventEmitter);